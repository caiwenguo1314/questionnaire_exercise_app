...
/* 定义一个Validation的状态，一个boolean的数组，根据input确认数组长度，并初始填入false  */
  const [inputValidation, setInputValidation] = useState<boolean[]>(
    Array(5).fill(false)
  );
  /* 创建一个检测input变化的函数 */
  const inputOnChangeFn = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const inputHandleOnChange = debounceFunction(inputOnChangeFn, 500);
  useEffect(() => {
